...
def on_open(ws, wsParam):
    def run(*args):
        ...
        print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists(r'data\demo.pcm'):
            os.remove(r'data\demo.pcm')
...
def run(text):
    ...
    os.system('ffplay -nodisp -ar 16000 -channels 1 -f s16le -i data\demo.pcm')
...
if __name__ == "__main__":
    wsParam = Ws_Param(
        ...
        APISecret = api_secret,
        APIKey = api_key,
        # Text = "我是讯飞的语音助手，有人感觉我说的声音很难听，我也是那么认为的，大家听一乐就好"
        Text = "123"
    )
    websocket.enableTrace(False)
    wsUrl = wsParam.create_url()
